...
def main():
    print("Loading wbo_parsed_events.json...")
    ...
    for ev in parsed_events:
        tid = get_thread_id(ev.get("Event thread link"))
        if tid:
            metadata_map[tid] = ev
    ...
    for e in extracted_data:
        ...
        tid = get_thread_id(e.get("link") or e.get("event_name"))
        if tid and tid in metadata_map:
